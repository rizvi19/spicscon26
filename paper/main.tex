...
Table~\ref{tab:setup} summarizes the tested configuration. Clean data use low base noise. WiFi-degraded periods add multipath-like noise bursts and shadowing. VLP-blocked periods attenuate optical observations. Mixed-dynamic periods alternate both degradation types. Results are aggregated over ten held-out seeds, 101--110. Across-seed uncertainty is reported as mean $\pm$ standard deviation. We report Euclidean mean error, 90th-percentile (P90) error, failure rate for errors above 2 m, and trajectory smoothness in meters per step.
...
Trajectory; repeated seeds & 100 steps; 101--110 \\
...
Table~\ref{tab:main} shows the expanded repeated-seed results. Static fusion is best in the clean scenario, with mean error $0.467\pm0.032$ m and P90 error $0.799$ m. VLP-only is best under WiFi degradation, with mean error $0.471\pm0.036$ m. In this controlled case, the optical modality remains clean, so a more elaborate fusion rule is unnecessary.

Adaptive fusion is most useful when the optical evidence is blocked or both modalities vary. Under VLP blockage, its mean error is $0.806\pm0.095$ m, compared with $1.078\pm0.116$ m for static fusion. Under mixed degradation, its mean error is $0.644\pm0.070$ m, compared with $0.811\pm0.104$ m for static fusion. Relative to static fusion, adaptive fusion improves mean error by 25.24\% and P90 error by 41.39\% in the VLP-blocked scenario. In the mixed-dynamic scenario, the corresponding improvements are 20.50\% and 35.94\%.

\begin{table*}[t]
\caption{Expanded Repeated-Seed Results (mean across ten held-out seeds; lower is better)}
\label{tab:main}
\centering
\scriptsize
\input{tables/expanded_seed_20260601/compact_main_results_expanded.tex}
\end{table*}
...
Table~\ref{tab:ablation} shows the robustness summary. Against static fusion, adaptive fusion lowers failure rate by 0.101 under VLP blockage and by 0.063 under mixed degradation. Smoothness improves by 48.91\% and 41.75\%, respectively. The five-seed and ten-seed mean-error comparison in Table~\ref{tab:stability} preserves the same difficult-scenario direction. These results support a conditional conclusion: adaptive weighting is useful when reliability changes, whereas clean or single-degradation settings may favor simpler estimators.
...
\centering
\tiny
\input{tables/expanded_seed_20260601/compact_ablation_expanded.tex}
\end{table}
\begin{table}[t]
\caption{Mean-Error Stability as Held-Out Seeds Expand (m; lower is better)}
\label{tab:stability}
\centering
\scriptsize
\input{tables/expanded_seed_20260601/compact_seed_stability.tex}
\end{table}
